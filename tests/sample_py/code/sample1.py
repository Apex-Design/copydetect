...
def generate_training_data_binary(num):
    if num == 1:
        ...
        print(
            "Incorrect num", num, "provided to generate_training_data_binary."
        )
        sys.exit()
...
def plot_training_data_binary(data, boundary=None):
    ...
    if boundary:
        ...
        for sv in s:
            plt.scatter(
                sv[0], sv[1], s=200, facecolors="none", edgecolors="black"
            )
    plt.show()
...
def plot_training_data_multi(data, boundaries=None, plot_margin=False):
    ...
    for item in data:
        plt.plot(
            item[0],
            item[1],
            colors[int(item[2]) - 1] + shapes[int(item[2]) - 1],
        )
    m = max(data.max(), abs(data.min())) + 1
    plt.axis([-m, m, -m, m])
