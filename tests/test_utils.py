...
class TestSmallDoc:
    ...
    def test_doc_overlap(self):
        ...
        overlap_1, overlap_2 = cd.find_fingerprint_overlap(
            fp1, fp2, idx1, idx2
        )

        assert np.array_equal(
            np.sort(overlap_1), np.array([0, 1, 2, 7, 8, 18])
        )
        assert np.array_equal(
            np.sort(overlap_2), np.array([5, 6, 7, 10, 16, 17])
        )

    def test_doc_overlap_boilerplate(self):
        fp1, idx1 = cd.get_document_fingerprints(
            self.doc1, 3, 1, self.boilerplate
        )
        fp2, idx2 = cd.get_document_fingerprints(
            self.doc2, 3, 1, self.boilerplate
        )
        overlap_1, overlap_2 = cd.find_fingerprint_overlap(
            fp1, fp2, idx1, idx2
        )
...
class TestTokenizerPythonSample:
    ...
    def test_copydetect(self):
        ...
        ol_idx1, ol_idx2 = cd.find_fingerprint_overlap(
            hashes1, hashes2, idx1, idx2
        )
        ...
        slices1 += offsets1[:, 1][
            np.clip(
                np.searchsorted(offsets1[:, 0], slices1),
                0,
                offsets1.shape[0] - 1,
            )
        ]
        slices2 += offsets2[:, 1][
            np.clip(
                np.searchsorted(offsets2[:, 0], slices2),
                0,
                offsets2.shape[0] - 1,
            )
        ]

        hl_code1, similarity1 = cd.highlight_overlap(
            self.sample_code, slices1, ">", "<"
        )
        hl_code2, similarity2 = cd.highlight_overlap(
            self.sample_copied_code, slices2, ">", "<"
        )
