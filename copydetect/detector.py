...
class CodeFingerprint:
    ...
    def __init__(
        ...
        try:
            ...
            if file is not None:
                # NOTE: errors='ignore' will ignore char that are not in utf8
                # so will lose some text
...
class CopyDetector:
    ...
    def _preprocess_code(self, file_list):
        ...
        file_data = {}
        for code_f in tqdm(
            file_list, bar_format="   {l_bar}{bar}{r_bar}", disable=self.silent
        ):
            try:
                cf = CodeFingerprint(
                    self.noise_t,
                ...
                logging.error(f"{code_f} caused the error.")
    ...
    def _comparison_loop(self):
        ...
        self.all_files = test_f_list + sorted(
            [f for f in self.ref_files if f not in self.test_files]
        )
        ...
        self.slice_matrix = [
            [np.array([]) for _ in range(len(self.all_files))]
        ...
        for i, test_f in enumerate(
            ...
            for j, ref_f in enumerate(self.all_files):
                if test_f not in self.file_data or ref_f not in self.file_data:
                    ...
                    self.all_files[i] not in self.test_files
                    or self.all_files[j] not in self.ref_files
                ):
                    continue
    ...
    def run(self):
        ...
        if len(self.test_files) == 0 or len(self.ref_files) == 0:
            ...
            logging.error(
                "Copy detector failed: No files found in "
